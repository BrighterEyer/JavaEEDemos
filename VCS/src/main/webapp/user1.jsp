...
<body>
	...
	<h1>
		用户列表
		<shiro:hasPermission name="user:add">--<a
				href="user/add.html">添加用户</a>
		</shiro:hasPermission>
	...
	<shiro:hasPermission name="user:query">所有用户列表<br />
	</shiro:hasPermission>
	<ul>
		<c:forEach items="${userList}" var="user">
			<li><shiro:hasPermission name="user:query">用户名：${user.username }</shiro:hasPermission>
				...
				<shiro:hasPermission name="user:update">----<a
						href="user/edit.html?id=${user.id}">修改用户</a>
				</shiro:hasPermission> <shiro:hasPermission name="user:del">----<a
						href="javascript:void(0);" class="del" ref="${user.id }">删除用户</a>
